                    ...
                    using (ZipArchive archive = new ZipArchive(zipToOpen, ZipArchiveMode.Update))
                    {
                        foreach (var contribution in approvedContributions)
                            foreach (var file in contribution.Files)
                                _ = archive.CreateEntryFromFile(file.URL, Path.Combine(contribution.
            ...
            ContributionStatus contributionStatus = ContributionStatus.Pending)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    MailboxAddress from = new MailboxAddress("iMarketing system", "nguyenthanhphat.a7.2018@gmail.com");
